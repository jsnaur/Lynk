...
export default function HomeFeedScreen({ onTabPress, navigation }: HomeFeedScreenProps) {
  ...
  // Formats raw quest rows into the shape expected by PostCard
  ...
  const fetchQuests = async (forceRefresh = false) => {
    ...
    try {
      ...
      // Call getPersonalizedFeed with an optional fast-path callback so the UI
      // can show preliminary results immediately while the AI re-ranks in the background.
      const aiSortedQuests = await getPersonalizedFeed(
        lat,
        lon,
        undefined,
        (fastQuests) => {
          // Fast-path: render results as soon as the first batch arrives
      ...
      // Final pass: apply the fully AI-sorted result set
